...
export default function ThemeSelector({
  ...
  return (
    <div className="fixed ml-0 mt-40">
      {color.map((currentColor) => {
        const borderStyle =
          selectedColor === currentColor
            ? "border-4 border-black border-double"
            : null;
        return (
          <span
            className={`block h-10 w-10 cursor-pointer ${bgStyle[currentColor]} ${borderStyle}`}
            key={currentColor}
      ...
      <span
        className="bg-onNeutralBg h-6 w-10 rounded-full relative inline-flex items-center"
